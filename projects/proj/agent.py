...
class LearningAgent:
    def __init__(self, env, init_exp_prob=1.0, is_training=True):
        ...
        self.UPDATE_TF_BOARD_EVERY = 256
        ...
        with open('log.csv', 'w+') as csvfile:
            ...
            fieldnames = ['Global_step', 'Game_number', 'Traning_step', 'Loss', 'Last_action', 'Random_prob', 'Total_reward', '#_of_Random_actions', '#_of_Conscious_actions']
        ...
        self.diff = self._target - readout_action
        ...
        self.train_operation = tf.train.RMSPropOptimizer(self.learning_rate, momentum=self.GRADIENT_MOMENTUM).minimize(self.loss)
    ...
    def train(self):
        ...
        # display_images_at_random(previous_states, actions, is_random=False)
        ...
        if self.train_step % self.TARGET_NET_UPDATE_FREQUENCE == 0:
            ...
            print str(v1[0][0][0][0]) + "\t" + str(v2[0][0][0][0])
        ...
        self.agents_reward_per_action = self._session.run(self.target_network.logits, feed_dict={self.target_network._input: current_states})
        ...
        for i in range(len(mini_batch)):
            if mini_batch[i][4]:
                ...
                agents_expected_reward.append(
                    rewards[i] + self.discount_factor * np.max(self.agents_reward_per_action[i]))
    ...
    def save_average_reward(self):
        with open('reward_per_game.txt', 'a+') as f:
            f.write(str(self.game_number) + "\t" + str(self.total_reward / self.game_steps) + "\n")
        ...
        if done:
            ...
            with open('log.csv', 'a+') as csvfile:
                ...
                line = [str(step), str(self.game_number), str(self.train_step), \
                        ...
                        str(self.total_reward), str(self.number_of_random_actions), \
                        str(self.number_of_systematic_actions)]
                writer.writerow(line)

            self.save_average_reward()
            self.game_number += 1
            self.game_steps = 0
            self.reset_debug_variables()
            _ = self.reset_env()
